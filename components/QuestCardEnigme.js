...
class QuestCardEnigme extends Component {
  ...
  onClick = async () => {
    ...
    try {
      const accounts = await web3.eth.getAccounts();
      const data = await Quest(this.props.questAddress)
        .methods.withdrawBalanceQuest(this.state.recipientAddress)
  ...
  render(props) {
    return (
      <Card fluid>
        <Image
          src={
            this.props.questPicture
              ? this.props.questPicture
              : "https://www.graphicstyle.fr/wp-content/uploads/1500-design-futuriste.jpg"
          }
          wrapped
          ui={false}
        />
        <Card.Content>
          ...
          <Card.Meta>
            <span className="date">
              Contrat :{" "}
              {this.props.questAddress
                ? this.props.questAddress
                : " addresse du contract"}
            </span>
          </Card.Meta>
          <Card.Description>
            {this.props.questDescription
              ? this.props.questDescription
              : "Sans Description"}
          </Card.Description>
        </Card.Content>
              ...
              <Grid>
                <Grid.Row>
                  <Grid.Column width={6}>
                    ...
                    {/* <Icon name='user' /> */}
                    <p>
                      {this.props.questBalance
                        ? web3.utils.fromWei(this.props.questBalance, "ether")
                        : 0}{" "}
                      ETH{" "}
                    </p>
                  </Grid.Column>
                  <Grid.Column width={7}>
                    <Form
                      ...
                      error={!!this.state.errorMessageWithdraw}
                    >
                      <Input
                        label="addresse"
                        labelPosition="right"
                      ...
                      <Message
                        ...
                        content={this.state.errorMessageWithdraw}
                      />
                      <Button fluid loading={this.state.loadingWithdraw}>
                        Récupérer solde
                      </Button>
                    </Form>
                  </Grid.Column>
              ...
              <Form onSubmit={this.onSubmit} error={!!this.state.errorMessage}>
                <Form.Field>
                  <Input
                    ...
                    onChange={(event) =>
                      this.setState({ questPrice: event.target.value })
                    }
                  />
                </Form.Field>
                <Message
